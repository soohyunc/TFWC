...
proc finish {} {
	...
	exec grep HISTORY temp.tfwc > ./trace/tfwc_periodic(0.01).HISTORY &
	exec grep avg_interval temp.tfwc > ./trace/tfwc_periodic(0.01).AvgInt &
	exec grep cwnd_ temp.tfwc > ./trace/tfwc_periodic(0.01).cwnd &

	
	exec xgraph -m -x time -y cwnd -t "TFWC CWND Dynamics" ./trace/tfwc_cwnd.xg &
	exec perl ../../../../Chart-Graph-2/graph.pl -i "./trace/tfwc_cwnd.xg" -x "time" -y "cwnd" -t "TFWC CWND Dynamics w/ (1% periodic)" -o ./graph/tfwc_cwnd_1p_periodic.png -s linespoints &
