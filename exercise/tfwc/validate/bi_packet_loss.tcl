...
proc finish {} {
	...
	exec grep HISTORY temp.tfwc > ./trace/tfwc_bi_packet.HISTORY &
	exec grep avg_interval temp.tfwc > ./trace/tfwc_bi_packet.AvgInt &
	exec grep cwnd_ temp.tfwc > ./trace/tfwc_bi_packet.cwnd &

	
	exec xgraph -m -x time -y cwnd -t "TFWC CWND Dynamics" ./trace/tfwc_cwnd.xg &
	exec perl ../../../../Chart-Graph-2/graph.pl -i "./trace/tfwc_cwnd.xg" -x "time" -y "cwnd" -t "TFWC CWND Dynamics (bi-packet loss)" -o ./graph/tfwc_cwnd_bi_packet.png -s linespoints &
