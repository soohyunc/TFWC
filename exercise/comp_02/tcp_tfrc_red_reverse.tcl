...
# Usage: ns tcp_tfrc_red_reverse.tcl [number of TCP/TFRC sources] [bottleneck queue size] [simulation time] [random seed number]
#
...
if {$argc <= 1} {
	puts "Usage: ns tcp_tfrc_red_reverse.tcl \[number of TCP/TFRC sources\] \[bottleneck queue size\] \[simulation time\] \[random seed number\] \> temp.tfrc"
	exit
}
...
#
for {set i 1} {$i <= $src_num} {incr i} {
	$ns queue-limit $n(2) $tfrc_node($i)	5
}
...
proc finish {} {
	...
	exec gnuplot tcp_tfrc_red_thru.plt &
	...
	#exec awk -f tcp_q.awk trace/out.queue
        exec gnuplot tcp_tfrc_red_q.plt &

	#
        # LOSS RATE PLOT
	#
        exec awk -f tfrc_loss.awk trace/out.queue
        exec gnuplot tfrc_red_loss.plt &
	...
	#
        ...
        #exec xgraph -bg white -x time -y cwnd -t "TFRC CWND Dynamics" trace/tfrc_cwnd_01.xg trace/tfrc_cwnd_02.xg trace/tfrc_cwnd_03.xg trace/tfrc_cwnd_04.xg &
        exec gnuplot tfrc_red_cwnd.plt &

	exec grep TCPWIN temp.tfrc > trace/tcp_cwnd.tr
	...
	# Avg Loss Interval
	#
	exec grep avg_interval_ temp.tfrc > trace/tfrc_avg_int.tr
	exec awk -f tfrc_avg_int.awk trace/tfrc_avg_int.tr

	exec grep pkt_drop_in_avg_hist temp.tfrc > trace/tfrc_loss_in_hist.tr
	exec awk -f tfrc_loss_in_hist.awk trace/tfrc_loss_in_hist.tr
