...
# Usage: ns tcp_red_reverse.tcl [number of TCP sources] [bottleneck queue size] [simulation time] [random seed number]
#
...
if {$argc <= 1} {
	puts "Usage: ns tcp_red_reverse.tcl \[number of TCP sources\] \[bottleneck queue size\] \[simulation time\] \[random seed number\] \> temp.tcp"
	exit
}
...
#
for {set i 1} {$i <= $src_num} {incr i} {
	$ns queue-limit $n(2) $tcp_node($i)	5
}
