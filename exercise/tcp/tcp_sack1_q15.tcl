...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Sack1 w/ DT Q=15" temp.rands &
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for TCP Sack1 w/ DT Q=15" out0.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput for TCP Sack1 w/ DT Q=15" temp.thru &

		exit 0
		}


# Set the last byte's value
set last_bytes 0
