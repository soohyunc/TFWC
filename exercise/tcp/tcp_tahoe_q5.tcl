# TCP Tahoe w/ queue size from n2 to n3 is 5 
#
...
$ns at 2.5 "$ns detach-agent $n0 $tcp_tahoe; $ns detach-agent $n3 $sink"
...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Tahoe w/ DT Q=5" temp.rands &
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for TCP Tahoe w/ DT Q=5" out0.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruut for TCP Tahoe w/ DT Q=5" temp.thru &
