#----------------------------------------------------------/
# On comparison between TCP Vegas and TFRC
#----------------------------------------------------------/
#
...
#----------------------------------------------------------/
# Create 4 TCP/Vegas and 4 TFRC Agents
#----------------------------------------------------------/
set tcp01 [new Agent/TCP/Vegas]
set tcp02 [new Agent/TCP/Vegas]
set tcp03 [new Agent/TCP/Vegas]
set tcp04 [new Agent/TCP/Vegas]
...
Agent/TCP/Vegas set window_ 100
...
proc finish {} {
		...
		exec perl ../../ns-2.26/bin/raw2xg -s 0.01 -m 90 < tcp_seq04.temp > tcp_seq04.rands
                ...
                #----------------------------------------------------------/
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "Vegas/TFRC (all) w/ DT (Q=Default)" -geometry 1200x400 temp.rands &
                exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Vegas #1 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq01.rands &
                exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Vegas #2 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq02.rands &
                exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Vegas #3 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq03.rands &
                exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Vegas #4 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq04.rands &
                ...
                #----------------------------------------------------------/
                exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for Vegas and TFRC (all) w/ DT (Q=Default)" -geometry 1200x400 out0.tr out1.tr &
                                                                                
                #----------------------------------------------------------/
                # Combined Thruput Plot
                #----------------------------------------------------------/
                exec awk -f thru.awk out.queue
                # exec xgraph -P -x time -y "rate (Mb/s)" -t "Combined Thruput for Vegas and TFRC (all) w/ DT (Q=Default)" -geometry 1200x400 temp.thru &
                ...
                exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput (4 Vegas & 4 TFRC) w/ DT (Q=Default)" -geometry 1200x400 tcp_thru.rands tf_thru.rands &
		...
		exec xgraph -P -x time -y "Loss Rate" -t "TCP/TFRC Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp_loss.rands tf_loss.rands &
		exec awk -f tcp01_loss.awk out.tr
#		exec xgraph -P -x time -y "Loss Rate" -t "Vegas #1 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp01_loss.rands &
		exec awk -f tcp02_loss.awk out.tr
#		exec xgraph -P -x time -y "Loss Rate" -t "Vegas #2 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp02_loss.rands &
		exec awk -f tcp03_loss.awk out.tr
#		exec xgraph -P -x time -y "Loss Rate" -t "Vegas #3 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp03_loss.rands &
		exec awk -f tcp04_loss.awk out.tr
#		exec xgraph -P -x time -y "Loss Rate" -t "Vegas #4 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp04_loss.rands &

		exec xgraph -P -x time -y "Loss Rate" -t "Vegas Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp01_loss.rands tcp02_loss.rands tcp03_loss.rands tcp04_loss.rands &
