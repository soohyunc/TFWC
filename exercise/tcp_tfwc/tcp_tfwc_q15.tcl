...
proc finish {} {
	...
	exec xgraph -P -x time -y "rate (Mb/s)" -t "Aggregated Thruput (4Sack & 4TFWC) Q=15" trace/tcp_thru.xg trace/tfwc_thru.xg &
	exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_thru.xg,trace/tfwc_thru.xg" -x "time" -y "rate (Mb/s)" -t "Aggregated Thruput (4Sack & 4TFWC) Q=15" -o graph/aggr_thru_sack_tfwc_q15.png -s linespoints &

	# INSTANEOUS QUEUE SIZE PLOT
	exec awk -f q_size.awk trace/out.queue
	exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_q.xg,trace/tf_q.xg" -x time -y "Q size" -t "Aggregated Queue Size (4Sack & 4TFWC) Q=15" -o graph/aggr_q_sack_tfwc_q15.png -s linespoints &

        # LOSS RATE PLOT
        exec awk -f tcp_loss.awk trace/out.queue
        exec awk -f tf_loss.awk trace/out.queue
        exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_loss.xg,trace/tf_loss.xg" -x time -y "Loss Rate" -t "Aggregated 4Sack/4TFWC Loss Rate, Q=15" -o graph/aggr_loss_sack_tfwc_q15.png -s linespoints &
