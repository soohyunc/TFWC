...
$ns at 100.0 "$ns detach-agent $n1 $tcp01; $ns detach-agent $n3 $tcp_sink01"
$ns at 100.0 "$ns detach-agent $n1 $tcp02; $ns detach-agent $n3 $tcp_sink02"
$ns at 100.0 "$ns detach-agent $n1 $tcp03; $ns detach-agent $n3 $tcp_sink03"
$ns at 100.0 "$ns detach-agent $n1 $tcp04; $ns detach-agent $n3 $tcp_sink04"

$ns at 100.0 "$ns detach-agent $n1 $tfwc01; $ns detach-agent $n3 $tfwc_sink01"
$ns at 100.0 "$ns detach-agent $n1 $tfwc02; $ns detach-agent $n3 $tfwc_sink02"
$ns at 100.0 "$ns detach-agent $n1 $tfwc03; $ns detach-agent $n3 $tfwc_sink03"
$ns at 100.0 "$ns detach-agent $n1 $tfwc04; $ns detach-agent $n3 $tfwc_sink04"
...
proc finish {} {
	...
	exec xgraph -P -x time -y "rate (Mb/s)" -t "Aggregated Thruput (4Sack & 4TFWC) Q=50" trace/tcp_thru.xg tfwc_thru.xg &
	exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_thru.xg,trace/tfwc_thru.xg" -x "time" -y "rate (Mb/s)" -t "Aggregated Thruput (4Sack & 4TFWC) Q=50" -o graph/aggr_thru_sack_tfwc_dq.png -s linespoints &

	# INSTANEOUS QUEUE SIZE PLOT
	exec awk -f q_size.awk trace/out.queue
	exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_q.xg,trace/tf_q.xg" -x time -y "Q size" -t "Aggregated Queue Size (4Sack & 4TFWC) Q=50" -o graph/aggr_q_sack_tfwc_dq.png -s linespoints &

	# LOSS RATE PLOT
	exec awk -f tcp_loss.awk trace/out.queue
	exec awk -f tf_loss.awk trace/out.queue
	exec perl ../../../Chart-Graph-2/graph.pl -i "trace/tcp_loss.xg,trace/tf_loss.xg" -x time -y "Loss Rate" -t "Aggregated 4Sack/4TFWC Loss Rate, Q=50" -o graph/aggr_loss_sack_tfwc_dq.png -s linespoints &
