# TCP NewReno w/ default DropTail queue
#
...
$ns connect $tcp $sink
$ns at 2.0 "$ns detach-agent $n0 $tcp; $ns detach-agent $n3 $sink"
...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP NewReno w/ DT Q=default" temp.rands &
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for TCP NewReno w/ default DT queue" out0.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput for TCP NewReno w/ default DT queue" temp.thru &
