...
#Agent/TCPSink set window_ 1000
...
$ns connect $tcp $sink
$ns at 3.25 "$ns detach-agent $n0 $tcp; $ns detach-agent $n3 $sink"
...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Reno w/ DT Q=10" temp.rands &
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for TCP Reno w/ DT Q=10" out0.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput for TCP Reno w/ DT Q=10" temp.thru &
...
proc record {} {
		...
		# Caluate the goodput/thruput (Mb/s) and write it to the file
		puts $f0 "$now [expr ($bw0-$last_bytes)/$time*8/1000000]"
		#puts $f1 "$now [expr (($bw0+$bw1)-$last_bytes))/$time*8/1000000]"
