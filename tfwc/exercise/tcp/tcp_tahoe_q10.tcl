# TCP Tahoe w/ queue size from n2 to n3 is 10 
#
...
$ftp attach-agent $tcp_tahoe
$ns at 0.3 "$ftp start"

$ns connect $tcp_tahoe $sink
$ns at 2.0 "$ns detach-agent $n0 $tcp_tahoe; $ns detach-agent $n3 $sink"
...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Tahoe w/ DT Q=10" temp.rands &
	
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for TCP Tahoe w/ DT Q=10" out0.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput for TCP Tahoe w/ DT Q=10" temp.thru &
...
proc record {} {
		global f0 f1 tcp_tahoe last_bytes
		set ns [Simulator instance]
		set time 0.05

		# Number of bytes
		set bw0 [$tcp_tahoe set ndatabytes_]
		set bw1 [$tcp_tahoe set nrexmitbytes_]
		...
		# Reset the bytes value on the traffic sinks
		set last_bytes $bw0
		#set last_bytes $bw1

		# Re-schedule the procudure
