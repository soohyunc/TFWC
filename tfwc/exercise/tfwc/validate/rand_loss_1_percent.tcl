...
$ns at 200.0 "$ns detach-agent $n1 $tfwc; $ns detach-agent $n3 $tfwc_sink"
...
proc finish {} {
	...
	#exec xgraph -bb -tk -nl -m -x time -y packets -t "TFWC Test" ./trace/temp.xg &
	#exec awk -f thru.awk ./trace/queue.tr
	#exec xgraph -P -x time -y rate -t "TFWC Test" ./trace/thru.xg &


        exec grep HISTORY temp.tfwc > ./trace/tfwc_random(0.01).HISTORY &
        exec grep avg_interval temp.tfwc > ./trace/tfwc_random(0.01).AvgInt &
        exec grep cwnd_ temp.tfwc > ./trace/tfwc_random(0.01).cwnd &
	...
	exec perl ../../../../Chart-Graph-2/graph.pl -i "./trace/tfwc_cwnd.xg" -x "time" -y "cwnd" -t "TFWC CWND Dynamics w/ (1% random)" -o ./graph/tfwc_cwnd_1p_random.png -s linespoints &
