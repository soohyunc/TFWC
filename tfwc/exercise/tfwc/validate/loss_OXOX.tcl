...
proc finish {} {
	...
	exec grep HISTORY temp.tfwc > ./trace/tfwc_OXOX.HISTORY &
	exec grep avg_interval temp.tfwc > ./trace/tfwc_OXOX.AvgInt &
	exec grep cwnd_ temp.tfwc > ./trace/tfwc_OXOX.cwnd &

	
	exec xgraph -m -x time -y cwnd -t "TFWC CWND Dynamics" ./trace/tfwc_cwnd.xg &
	exec perl ../../../../Chart-Graph-2/graph.pl -i "./trace/tfwc_cwnd.xg" -x "time" -y "cwnd" -t "TFWC CWND Dynamics (OXOX...like packet loss)" -o ./graph/tfwc_cwnd_OXOX.png -s linespoints &
