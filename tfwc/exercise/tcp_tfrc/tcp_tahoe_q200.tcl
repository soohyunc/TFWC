...
proc finish {} {
		...
		exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Tahoe w/ DT Q=200" -geometry 1200x400 temp.rands &
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Tx rate for 4xTCP Ta hoe and 4xTFRC w/ DT (Q=200)" -geometry 1200x400 out0.tr out1.tr &

		# thruput calculation
		exec awk -f thru.awk out.queue
		exec xgraph -P -x time -y "rate (Mb/s)" -t "Thruput for 4xTCP Tahoe and 4xTFRC w/ DT (Q=200)" -geometry 1200x400 temp.thru &

		exec awk -f tcp_thru.awk out.queue
		exec awk -f tf_thru.awk out.queue
                exec xgraph -P -x time -y "rate (Mb/s)" -t "Individual Thruput (4xTCP Tahoe and 4xTFRC) w/ DT (Q=200)" -geometry 1200x400 tcp_temp.thru tf_temp.thru &
