#----------------------------------------------------------/
# On comparison between TCP Newreno and TFRC
#----------------------------------------------------------/
#
...
#----------------------------------------------------------/
# Create 4 TCP/Newreno and 4 TFRC Agents
#----------------------------------------------------------/
set tcp01 [new Agent/TCP/Newreno]
set tcp02 [new Agent/TCP/Newreno]
set tcp03 [new Agent/TCP/Newreno]
set tcp04 [new Agent/TCP/Newreno]
...
Agent/TCP/Newreno set window_ 100
...
proc finish {} {
		...
		#exec perl ../../ns-2.26/bin/raw2xg -s 0.01 -m 90 < tcp_seq04.temp > tcp_seq04.rands
                ...
                #----------------------------------------------------------/
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "Newreno/TFRC (all) w/ DT (Q=Default)" -geometry 1200x400 temp.rands &
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Newreno #1 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq01.rands &
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Newreno #2 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq02.rands &
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Newreno #3 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq03.rands &
                #exec xgraph -bb -tk -nl -m -x time -y packets -t "TCP Newreno #4 w/ DT (Q=Default)" -geometry 1200x400 tcp_seq04.rands &


                #exec xgraph -bb -tk -nl -m -x time -y packets -t "TFRC #1 w/ DT (Q=Default)" -geometry 1200x400 tf_seq01.rands &
                ...
                #----------------------------------------------------------/
                #exec xgraph -P -x time -y "rate (Mb/s)" -t "Aggregated/Combined Tx rate (Newrenos and TFRCs) w/ DT (Q=Default)" -geometry 1200x400 out0.tr out1.tr &
                                                                                
                #----------------------------------------------------------/
                # Aggregated/Combined Thruput Plot
                #----------------------------------------------------------/
                #exec awk -f thru.awk out.queue
                # exec xgraph -P -x time -y "rate (Mb/s)" -t "Aggregated/Combined Thruput (4Newreno & 4TFRC) w/ DT (Q=Default)" -geometry 1200x400 temp.thru &
                ...
                exec xgraph -P -x time -y "rate (Mb/s)" -t "Aggregated Thruput (4Newreno & 4TFRC) w/ DT (Q=Default)" -geometry 1200x400 tcp_thru.rands tf_thru.rands &

		exec perl ../../../Chart-Graph-2/graph.pl -i "tcp_thru.rands,tf_thru.rands" -x "time" -y "rate (Mb/s)" -t "Aggregated Thruput (4Newreno & 4TFRC) w/ DT (Q=default)" -d 1200x400 -o ./graph/aggr_thru_newreno.png -s linespoints & 
		...
		exec xgraph -P -x time -y "Loss Rate" -t "Aggregated 4Newreno/4TFRC Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp_loss.rands tf_loss.rands &

		exec perl ../../../Chart-Graph-2/graph.pl -i "tcp_loss.rands,tf_loss.rands" -x "time" -y "Loss Rate" -t "Aggregated 4Newreno/4TFRC Loss Rate w/ DT (Q=default)" -d 1200x400 -o ./graph/aggr_loss_newreno.png -s linespoints &


		#exec awk -f tcp01_loss.awk out.tr
		#exec xgraph -P -x time -y "Loss Rate" -t "Newreno #1 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp01_loss.rands &
		#exec awk -f tcp02_loss.awk out.tr
		#exec xgraph -P -x time -y "Loss Rate" -t "Newreno #2 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp02_loss.rands &
		#exec awk -f tcp03_loss.awk out.tr
		#exec xgraph -P -x time -y "Loss Rate" -t "Newreno #3 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp03_loss.rands &
		#exec awk -f tcp04_loss.awk out.tr
		#exec xgraph -P -x time -y "Loss Rate" -t "Newreno #4 Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp04_loss.rands &

		#exec xgraph -P -x time -y "Loss Rate" -t "Newreno Loss Rate w/ DT (Q=Default)" -geometry 1200x400 tcp01_loss.rands tcp02_loss.rands tcp03_loss.rands tcp04_loss.rands &

                #----------------------------------------------------------/
                # Instaneous Queue Size
                #----------------------------------------------------------/
                exec awk -f q_size.awk out.queue
                exec xgraph -P -x time -y "Q size" -t "Aggregated/Combined 4Newreno/4TFRC Q-size w/ DT (Q=Default)" -geometry 1200x400 q_size.rands &

		exec perl ../../../Chart-Graph-2/graph.pl -i "q_size.rands" -x time -y "Q size" -t "Aggregated/Combined 4Newreno/4TFRC Q-size w/ DT (Q=Default)" -d 1200x400 -o ./graph/all_newreno_q_size_default.png -s linespoints &

                exec xgraph -P -x time -y "Q size" -t "Aggregated 4Newreno/4TFRC Q-size w/ DT (Q=Default)" -geometry 1200x400 tcp_q.rands tf_q.rands &

		exec perl ../../../Chart-Graph-2/graph.pl -i "tcp_q.rands,tf_q.rands" -x time -y "Q size" -t "Aggregated 4Newreno/4TFRC Q-size w/ DT (Q=Default)" -d 1200x400 -o ./graph/aggr_q_newreno.png -s linespoints &
		...
		exec xgraph -P -x time -y RTT -t "Aggregated 4TFRC RTT w/ DT (Q=Default)" -geometry 1200x400 tf_rtt.rands &

		exec perl ../../../Chart-Graph-2/graph.pl -i "tf_rtt.rands" -x time -y RTT -t "Aggregated 4TFRC RTT w/ DT (Q=Default)" -d 1200x400 -o ./graph/aggr_tfrc_rtt_newreno_q_default.png -s linespoints &
